...
BEGIN
	...
	CONVERT(VARCHAR,DECRYPTBYASYMKEY(ASYMKEY_ID(NV.PUBKEY), DIEMTHI,CONVERT(NVARCHAR,@MATKHAU))) DIEM,
	BD.MAHP
	FROM BANGDIEM BD JOIN SINHVIEN SV ON BD.MASV = SV.MASV JOIN HOCPHAN HP ON BD.MAHP = HP.MAHP 
	JOIN LOP L ON L.MALOP = SV.MALOP JOIN NHANVIEN NV ON NV.MANV = L.MANV
	WHERE SV.MALOP = @MALOP
END
GO
EXEC SP_UV_BANGDIEM 'LOP01','123456'
GO

--PROCEDURE CUA PHUC NHA========================================
...
GO
--PROC CUA LONG NHA =================================================
-- PROCEDURE LẤY TOÀN BỘ SINH VIÊN CỦA LỚP 
CREATE PROC SP_UV_SV_LOP @MALOP VARCHAR(20)
AS
	--SET DATEFORMAT DMY
	SELECT SV.MASV, SV.HOTEN, SV.NGAYSINH, SV.DIACHI, L.MANV
	FROM LOP L JOIN SINHVIEN SV ON L.MALOP = SV.MALOP
	WHERE SV.MALOP = @MALOP
GO
EXEC SP_UV_SV_LOP 'LOP02'
GO
CREATE PROC SP_UPD_SV @MASV VARCHAR(20), @HOTEN NVARCHAR(100), @NGAYSINH DATETIME, @DIACHI NVARCHAR(200)
AS
	SET DATEFORMAT MDY
	UPDATE SINHVIEN 
	...
	WHERE MASV = @MASV
GO
--EXEC SP_UPD_SV 'SV01', N'Trần Văn H', '5/2/1999', N'03 Trần Đại Nghĩa'